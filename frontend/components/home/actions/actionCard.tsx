...
export default function ActionCard({ title, description, icon, href }) {
  return (
    <Link
      href={href}
      className={`w-full group border-b-[4px] border-white hover:border-accent hover:bg-accent-50 bg-[var(--background-color)] flex flex-col items-center py-10 lg:py-16  px-5 lg:px-10 text-center rounded-[10px]`}
    >
      {icon && (
